...
```{r}
mi_10 <- function(baseline.model, min.mi = 10, data){
...
```{r}
mi_10_whileloop <- function(baseline.model, min.mi = 10, data){
...
```{r}
mi_10_whileloop(baseline.model = model, min.mi = 10, data = HolzingerSwineford1939)
```

Are the MI values really above 10 after adding the modifications?
```{r}
