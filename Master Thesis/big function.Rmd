...
```{r}
mi_whileloop(baseline.model = model, min.mi = 11, data = HolzingerSwineford1939)
```
...
mi_cv <- function(baseline.model, data, split, ...){
  ...
  ifelse(chisq.diff$`Pr(>Chisq)`[2] < .87, sign <- TRUE, sign <- FALSE)
  
  
  
  # Creating chisq.diff object: ???????????????????????????????????????????
  # empty.model <- " "
  # empty.fit <- lavaan::cfa(empty.model, test)
  # 
  # sign <- NULL
