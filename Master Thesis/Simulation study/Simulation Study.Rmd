...
latentLoadings <- matrix(diag(2), 2)
latentLoadings[1, 2] <- NA
latentLoadings[2, 1] <- NA
latentLoadingValues <- matrix(0, 2, 2)
latentLoadingValues[1, 2] <- 0.7
latentLoadingValues[2, 1] <- 0.7
RPS <- bind(latentLoadings, latentLoadingValues)
RPS

my.model <- simsem::model(LY = LY, RTE = RTE, RPS = RPS, modelType = "CFA")
```

Creating a matrix of varying conditions of lambda, rho and delta:
```{r}
lambda <- c(0.1, 0.4, 0.7)
rho    <- c(0.1, 0.4, 0.7)
delta  <- c(0.1, 0.4, 0.7)
